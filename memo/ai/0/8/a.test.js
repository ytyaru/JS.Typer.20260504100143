...
describe('a.js', ()=>{
    ...
    describe('a.p', ()=>{
        ...
        describe('.num', ()=>{
            ...
            describe('.inf', ()=>{
                test.each([[Infinity],[-Infinity]])(`(%p)->T`, v=>expect(a.p.num.inf(v)).toBe(true));
                test.each(Data.num(Infinity,-Infinity))(`(%p)->F`, v=>expect(a.p.num.inf(v)).toBe(false));
            });
            describe('.unsafe', ()=>{
                test.each([[Number.MAX_SAFE_INTEGER+1],[Number.MIN_SAFE_INTEGER-1]])(`(%p)->T`, v=>expect(a.p.num.unsafe(v)).toBe(true));
                test.each([[Number.MAX_SAFE_INTEGER],[Number.MIN_SAFE_INTEGER]])(`(%p)->F`, v=>expect(a.p.num.unsafe(v)).toBe(false));
            });
            describe('.fin', ()=>{
                test.each([[Number.MAX_SAFE_INTEGER],[Number.MIN_SAFE_INTEGER]])(`(%p)->F`, v=>expect(a.p.num.fin(v)).toBe(true));
                test.each([[Number.MAX_SAFE_INTEGER-0.1],[Number.MIN_SAFE_INTEGER+0.1]])(`(%p)->F`, v=>expect(a.p.num.fin(v)).toBe(true));
//                test.each([[Number.MAX_SAFE_INTEGER+0.1],[Number.MIN_SAFE_INTEGER-0.1]])(`(%p)->F`, v=>expect(a.p.num.fin(v)).toBe(false));
                test.each([[Number.MAX_SAFE_INTEGER+1],[Number.MIN_SAFE_INTEGER-1]])(`(%p)->F`, v=>expect(a.p.num.fin(v)).toBe(false));
            });
            describe('.flt', ()=>{
                test.each([[0.1],[-0.1]])(`(%p)->T`, v=>expect(a.p.num.flt(v)).toBe(true));
//                test.each([[Number.MAX_SAFE_INTEGER-1.25],[Number.MIN_SAFE_INTEGER+1.25]])(`(%p)->T`, v=>expect(a.p.num.flt(v)).toBe(true));
                test.each([[Number.MAX_SAFE_INTEGER],[Number.MIN_SAFE_INTEGER]])(`(%p)->F`, v=>expect(a.p.num.flt(v)).toBe(false));
//                test.each([[Number.MAX_SAFE_INTEGER+0.1],[Number.MIN_SAFE_INTEGER-0.1]])(`(%p)->F`, v=>expect(a.p.num.flt(v)).toBe(false));
                test.each([[Number.MAX_SAFE_INTEGER+1],[Number.MIN_SAFE_INTEGER-1]])(`(%p)->F`, v=>expect(a.p.num.flt(v)).toBe(false));
            });
            describe('.int', ()=>{
                test.each([[0.1],[-0.1]])(`(%p)->F`, v=>expect(a.p.num.int(v)).toBe(false));
                test.each([[Number.MAX_SAFE_INTEGER],[Number.MIN_SAFE_INTEGER]])(`(%p)->T`, v=>expect(a.p.num.int(v)).toBe(true));
                test.each([[Number.MAX_SAFE_INTEGER+1],[Number.MIN_SAFE_INTEGER-1]])(`(%p)->F`, v=>expect(a.p.num.int(v)).toBe(false));
                describe('.bit', ()=>{
                    test.each([['1'],[0],[54],[0.5],[-1]])('(%p)->E',v=>expect(()=>a.p.num.int.bit(0,v)).toThrow());
                    test.each([[1],[53]])('(%p)->T',v=>expect(a.p.num.int.bit(0,v)).toBe(true));
                    test('1M(%p)->T',()=>expect(a.p.num.int.bit(0,1)).toBe(true));
                    test('1m(%p)->T',()=>expect(a.p.num.int.bit(-1,1)).toBe(true));
                    test('1M(%p)->F',()=>expect(a.p.num.int.bit(1,1)).toBe(false));
                });
            });
            describe('.uint', ()=>{
                test.each([[0.1],[-0.1]])(`(%p)->F`, v=>expect(a.p.num.uint(v)).toBe(false));
                test.each([[0],[Number.MAX_SAFE_INTEGER]])(`(%p)->T`, v=>expect(a.p.num.uint(v)).toBe(true));
                test.each([[-1],[Number.MIN_SAFE_INTEGER],[Number.MIN_SAFE_INTEGER]])(`(%p)->F`, v=>expect(a.p.num.uint(v)).toBe(false));
                test.each([[Number.MAX_SAFE_INTEGER+1],[Number.MIN_SAFE_INTEGER-1]])(`(%p)->F`, v=>expect(a.p.num.uint(v)).toBe(false));
            });
        });
        describe('.big', ()=>{
...
//            test.each(D)('false B(%p)', (v) => expect(a.p.sym(new Symbol(v))).toBe(false));
    ...
    describe('a.r', ()=>{
        ...
        describe('.cal', ()=>{
            ...
            describe('.method', ()=>{
                ...
                test.each(Data.fn('method'))(`(%p)->F`, v=>expect(a.r.cal.method(v)).toBe(false));
            });
            describe('.native', ()=>{
    ...
    describe('a.invalid', ()=>{
        ...
        test.each(Data.without('dic'))('(%p)->F', (v) => expect(a.invalid(v)).toBe(false));
    });
});

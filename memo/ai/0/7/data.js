...
export class Data {
    ...
    // cartesian([this.#C, this.#c], 'g s gs v f'.split(' '))=>[o,k]
    // cartesian([this.#C, this.#c], 'g s gs v f'.split(' ')).map(([o,k])=>[o,(/[A-Z]+]/.test(k) ? 's' : '')+k)
...
//    static insDes() {return cartesian([this.#C, this.#c], 'g s gs v f'.split(' ')).map(([o,k])=>[o,(/[A-Z]+]/.test(k) ? 's' : '')+k).map(([o,k])=>[Object.getOwnPropertyDescriptor(o,k)])}
//    static clsDes() {return [this.#C,this.#D].flatMap(o=>this.#getDes(o))}
//    static insDes() {return [this.#c,this.#d].flatMap(o=>this.#getDes(Object.getPrototypeOf(o)))}
//    static #getDes(o) {return 'g s gs v f'.split(' ').map(k=>[Object.getOwnPropertyDescriptor(o,(/[A-Z]+/.test(o?.constructor?.name) ? 's' : '')+k)])}
    static clsDes() {return [this.#C,this.#D].flatMap(o=>this.#getDes(o,'s'))}
    static insDes() {return [this.#c,this.#d].flatMap(o=>this.#getDes(Object.getPrototypeOf(o)))}
//    static #getDes(o,p='') {return 'g s gs v f'.split(' ').map(k=>[Object.getOwnPropertyDescriptor(o,p+k)])}

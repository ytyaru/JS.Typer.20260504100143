...
class HasKeysVerifier {
  ...
  static #validateId(id) {
    if (!this.#validIds.includes(id)) {
      throw new Error(`不正な id (パス) が検出されました。実際値: ${id}, 期待値: ${this.#validIds.join(', ')}。`);
    }
  }
  static #validateTypeSpec(keys, id) {
    if (0===keys.length) {
      throw new Error(`${id} のプロパティ名(...keys)が指定されていません。存在を確認したいキー名を1つ以上指定してください。`);
    }
    if (!keys.every(k => a.p.str(k) || a.p.sym(k))) {
      throw new Error(`${id} のプロパティ名に不正な型が渡されました。キー名はすべてStringまたはSymbol型で指定してください。`);
    }
  }
...
//const isClsFmt = (v)=> /^class\b/.test(fnSrc(v));
//const isFnFmt = (v)=> /^(async\s+)?function(\s|\*|\()/.test(fnSrc(v));
//const isArrFmt = (v)=> /^(async\s*)?(\([^)]*\)|[a-zA-Z_$][\w_$]*)\s*=>/.test(fnSrc(v));
/*
const calFmt = {
    cls: v=>/^class\b/.test(fnSrc(v)),
    fn: v=>/^(async\s+)?function([\s*()|$])/.test(fnSrc(v)),
    arr: v=>/^(async\s*)?(\([^)]*\)|[a-zA-Z_$][\w_$]*)\s*=>/.test(fnSrc(v)),
    nat: v=>fnSrc(v).includes('[native code]'),
    bou: v=>v.name.startsWith('bound '),
}
*/
//const isCalFmt = (v,t)=>Object.keys(calFmt).every(k=>calFmt[k](v)===(k===t));
const calFmt = {
...
a.r.cal.native = (v)=> a.r.cal(v) && fnSrc(v).includes('[native code]') && !v.name.startsWith('bound ');
a.r.cal.bound = (v)=> a.r.cal(v) && fnSrc(v).includes('[native code]') && v.name.startsWith('bound ');
//a.r.cal.native = (v)=> a.r.cal(v) && isCalFmt(v,'nat');
//a.r.cal.bound = (v)=> a.r.cal(v) && isCalFmt(v,'bou');
//a.r.cal.fn = (v)=> a.r.cal(v) && !a.r.cal.arrow(v) && !a.r.cal.method(v) && !a.r.cal.native(v) && !a.r.cal.bound(v);
a.r.cal.fn = (v)=> a.r.cal(v) && isCalFmt(v,'fn') && !a.r.cal.native(v) && !a.r.cal.bound(v);

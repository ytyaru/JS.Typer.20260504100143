...
class Caller {
  ...
  static _is(v) {return isFn(v) && !isCls(v)}
  static _isFn(v,s) {return /^(async\s+)?function([\s*()|$])/.test(s)}
  ...
  static _isMethod(v,s) {return ['Fn','Arrow','Native','Bound'].every(n=>!this[`_is${n}`](v,s))}
  static #getKind(v,s) {
    if (this.#isFn(v,s)) return 'Function';
    else if (this.#isArrow(v,s)) return 'Arrow';
    else if (this.#isBound(v,s)) return 'Bound';
    else if (this.#isNative(v,s)) return 'Native';
    else if (this.#isMethod(v,s)) return 'Method';
...
//    return [a:T.includes('Async') ? 'Async' : '', g:T.includes('Generator')];
  ...
  static isFn(v) {return this.#is(v) && this._isFn(v,this.#fnSrc(v))}

...
export const getTag = (...V)=>{
    ...
    else if (isO(v) && null===Object.getPrototypeOf(v)) return `Dictionary`;
    else return isIns(v) ? `Instance<${v.constructor.name}>`: tag(v);
}
class Typer {
...
export class Typ {
    ...
    is(...args) {return Typer.execute(this._.isThrow, false, false, ...args);}
